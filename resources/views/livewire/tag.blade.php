<div class="flex justify-center md:mt-[2rem] mt-[1rem]">
        ...
        <form class="md:mt-4 mt-3" wire:submit.prevent="print">
            ...
            <div class="grid xl:grid-cols-2 xl:gap-6">
                ...
                <div class="relative z-0 mb-6 w-full group">
                    ...
                    <label for="destino"
                           class="absolute text-sm text-gray-500 text-gray-600 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:text-gray-900 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Destino</label>
                </div>
                ...
                <div class="relative z-0 mb-6 w-full group">
                    ...
                    <input type="text" name="remetente" id="remetente"
                           class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-red-300 appearance-none text-dark border-gray-500 focus:border-red-500 focus:border-opacity-60 focus:outline-none focus:ring-0 focus:border-red-500 peer"
                           placeholder=" " required="" wire:model.defer="remetente">
                    <label for="remetente"
                           class="absolute text-sm text-gray-500 text-gray-600 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:text-gray-900 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Remetente</label>
                </div>
            </div>
    ...
    @section('js')
        <script>
            window.addEventListener('print', function (evt) {
                var pdf = $('<iframe />');
                pdf[0].name = "pdf";
                pdf.css({"position": "absolute", "top": "-1000000px"});
                $("body").append(pdf);
                var frameDoc = pdf[0].contentWindow ? pdf[0].contentWindow : pdf[0].contentDocument.document ? pdf[0].contentDocument.document : pdf[0].contentDocument;
                frameDoc.document.open();
                frameDoc.document.write(evt.detail);
